...
public class Blink1 {
    ...
    /**
     * pointer to native hid_device* do not use.
     */
    long blink1DevicePtr = 0;

    /**
     * Error code of last operation 0 = ok, anything else = error
     */
    int errorCode = 0;

    private static Logger logger = LoggerFactory.getLogger(Blink1.class);
    ...
    public static void main(String args[]) {
        ...
        if (count == 0) {
            logger.info("-> no devices found: continuing for troubleshooting");
        }

        logger.info("-> device id : serial : path:");
        String paths[] = Blink1.getDevicePaths();
        String serials[] = Blink1.getDeviceSerials();
        for (int i = 0; i < paths.length; i++) {
            logger.info("-> " + i + ": " + serials[i] + " : " + paths[i]);
        }

        logger.info("Opening deviceId 0");

        Blink1 blink1 = Blink1.openById(0);

        if (blink1.error()) {
            logger.info("-> error on open(): no blink(1), next call will return error ");
        }

        logger.info("-> fading to 10,20,30 ");
        rc = blink1.fadeToRGB(100, 10, 20, 30);
        logger.info("-> return code = " + rc);

        int ver = blink1.getFirmwareVersion();
        logger.info("-> firmware version: " + ver);
        ...
        if (serials.length >= 2) {
            ...
            logger.info("Opening two devices: " + serialA + " and " + serialB);
            Blink1 blink1A = Blink1.openBySerial(serialA);
            Blink1 blink1B = Blink1.openBySerial(serialB);
            logger.info("-> fading " + serialA + " to red");
            blink1A.fadeToRGB(100, 255, 0, 0);
            Blink1.pause(500);
            logger.info("-> fading " + serialB + " to green");
            blink1B.fadeToRGB(100, 0, 255, 0);
            Blink1.pause(500);
            logger.info("-> fading " + serialA + " to blue");
            blink1A.fadeToRGB(100, 0, 0, 255);
            Blink1.pause(500);
            logger.info("-> fading " + serialB + " to purple");
        ...
        for (int i = 0; i < 10; i++) {
            ...
            logger.info("-> setting device " + id + " to color " + r + "," + g + "," + b + "   ");

            blink1 = Blink1.openById(id);
            if (blink1.error()) {
                logger.info("-> couldn't open " + id + " ");
            }
        ...
        logger.info("Turn off all blink(1)s.");
    ...
    /**
     * Constructor. Normally not used. Use static methods Blink1.open...() to get a Blink1 object
     */
    public Blink1() {
    ...
    /**
     * Open the first (or only) blink(1) device. Causes an enumerate to happen. Stores open device
     * id statically in native lib.
     *
    ...
    /**
     * Open blink(1) device by blink(1) numerical id (0-getCount()). Id list is ordered by serial
     * number.
     *
    ...
    /**
     * Do a transaction with the Blink1. Length of both byte arrays determines amount of data sent
     * or received.
     *
     ...
     * @returns blink1_command response code, -1 == fail FIXME: this does not work correctly. Use
